...
function performSyncWorkOnRoot(root) {
  ...
  // 我们现在有一个一致的树。因为这是一个同步渲染，即使有些东西挂起了，我们也会提交它。 
...
function renderRootSync(root: FiberRoot, lanes: Lanes) {
  ...
  // 使用 do while 循环 开启工作循环 ，执行递归的操作 '递' 创建子 filer, '归' 创建真实 dom
  do {
    try {
      workLoopSync();
      break;
...
function performUnitOfWork(unitOfWork: Fiber): void {
  ...
  if (enableProfilerTimer && (unitOfWork.mode & ProfileMode) !== NoMode) {
    ...
    // 会得到 workInProgress.child 对象; 就是 unitOfWork 的 child
...
function completeUnitOfWork(unitOfWork: Fiber): void {
  ...
  do {
    ...
    // 继续看 该 fiber 是否还有 兄弟节点，有的话则继续 将 workInProgress 赋值为 siblingFiber，
    // 然后 return，结束当前 completeUnitOfWork 给 siblingFiber 进行 beginWork
    if (siblingFiber !== null) {
      // If there is more work to do in this returnFiber, do that next.
      workInProgress = siblingFiber;
      return;
    }
    // 如果 兄弟节点也没有了，则表示当前父节点下的所有 child 都生成了 真实 dom，返回父级的fiber
...
function commitRootImpl(root, renderPriorityLevel) {
  ...
  if (subtreeHasEffects || rootHasEffect) {
    ...
    // 提交阶段分为几个子阶段。 我们为每个阶段单独传递效果列表：所有突变效果都在所有布局效果之前，依此类推。

    // 第一阶段是“突变前”阶段。 在我们改变它之前，我们使用这个阶段来读取宿主树的状态。 这是调用 getSnapshotBeforeUpdate 的地方。
